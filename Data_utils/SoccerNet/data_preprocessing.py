...
def read_gt_file(ground_truth_path):
    ...
    ground_truth_data = raw_gt_data.loc[:, ['class', 'x', 'y', 'w', 'h', 'frame', 'area']].copy()
...
if __name__ == "__main__":
            ...
            if data_type in ['train', 'test']:
                ...
                for image in os.listdir(image_folder_path):
                    if '.jpg' in image:
                        ...
                        gt_image_data = gt_data.loc[gt_data['frame'] == image_number, :].copy()
                        gt_image_data.loc[gt_image_data['area'] == gt_image_data['area'].min(), 'class'] = 1
                        gt_image_data.drop(['frame', 'area'], axis=1, inplace=True)
    ...
    yaml_path_file = os.path.join(os.path.dirname(__file__), 'yaml_path.txt')
